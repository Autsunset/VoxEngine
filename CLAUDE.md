...
- `ReaderPlaybackPlanner` — **pure, tested** chunking/prefetch logic (`splitTextForTts`, `buildPrefetchWindow`, `normalizePosition`, `nextPosition`). `ReaderPlaybackService` **delegates** to these (no private duplicate). Role-aware variants `chunkKeysForPlaybackRoleAware` / `buildPrefetchWindowRoleAware` emit `RoleChunk(role, character, text)`; the plain `String` variants are kept for tests and equal the role-aware path with `roleEnabled=false`.
- `RoleSegmenter` — **pure, tested** (`RoleSegmenterTest`). Splits a paragraph into narration/dialogue `RoleSegment`s via a quote-state machine over `SpeechTextNormalizer`-normalized text, and extracts a speaker name from the narration prefix (`张三笑道：`→`张三`). `voiceFor(...)` resolves a segment to a voice via the active role profile, falling back to the book voice. The planner segments first, then length-splits each span.
- **Per-role voice routing** — when `reader_role_enabled` is on, `ReaderPlaybackService` resolves each `RoleChunk` to a voice (narration / dialogue / named-character map) and precomputes a per-voice "is clone/design" map so throttling (`conservativeRequestIntervalMs`) is applied per resolved voice, not globally. Disabled → single book voice, byte-identical chunking to before.
...
- **Room** `AppDatabase` (`data/`), version **5**, four tables: `voices`, `synthesis_history`, `reader_books`, `reader_chapters`. Uses **real `Migration`s (1→2→3→4→5)**, not destructive fallback — bump the version and add a migration for any schema change. `MIGRATION_1_2` and `MIGRATION_4_5` (voice metadata: `gender`/`ageGroup`/`tags`/`groupId`) use an `addColumnIfMissing` PRAGMA guard. Preset voices are **not** seeded into the DB — their gender/age metadata lives in `MiMoTTSClient.PRESET_VOICE_META`; custom voices carry DB metadata.
